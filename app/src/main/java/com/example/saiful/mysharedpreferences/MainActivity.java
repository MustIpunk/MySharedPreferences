package com.example.saiful.mysharedpreferences;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvPhoneNo = findViewById(R.id.tv_phone_number);
        tvEmail = findViewById(R.id.tv_email);
        tvIsLoveMU = findViewById(R.id.tv_love_mu);
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.btn_save) {
            Intent intent = new Intent(MainActivity.this, FromUserPreferenceActivity.class);
            if (isPreferenceEmpty) {
                intent.putExtra(FromUserPreferenceActivity.EXTRA_TYPE_FORM, FromUserPreferenceActivity.TYPE_ADD);
                intent.putExtra("USER", userModel);
            } else {
                intent.putExtra(FromUserPreferenceActivity.EXTRA_TYPE_FORM, FromUserPreferenceActivity.TYPE_EDIT);
                intent.putExtra("USER", userModel);
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE) {
            if (resultCode == FromUserPreferenceActivity.RESULT_CODE) {
                userModel = data.getParcelableExtra(FromUserPreferenceActivity.EXTRA_RESULT);
                populateView(userModel);
                checkForm(userModel);
            }
        }
